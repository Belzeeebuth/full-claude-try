...
function renderMetrics(client: Client): string {
  ...
  const lines = [
    '# HELP harvestbot_uptime_seconds Temps depuis le démarrage',
    '# TYPE harvestbot_uptime_seconds gauge',
    `harvestbot_uptime_seconds ${Math.round((Date.now() - metrics.startedAt) / 1_000)}`,
    '# HELP harvestbot_commands_total Commandes exécutées',
    '# TYPE harvestbot_commands_total counter',
    `harvestbot_commands_total ${metrics.commandsTotal}`,
    '# HELP harvestbot_command_errors_total Commandes en erreur',
    '# TYPE harvestbot_command_errors_total counter',
    `harvestbot_command_errors_total ${metrics.commandErrors}`,
    '# HELP harvestbot_interactions_total Interactions traitées',
    '# TYPE harvestbot_interactions_total counter',
    `harvestbot_interactions_total ${metrics.interactionsTotal}`,
    '# HELP harvestbot_guilds Serveurs Discord connectés (ce shard)',
    '# TYPE harvestbot_guilds gauge',
    `harvestbot_guilds ${client.guilds.cache.size}`,
    '# HELP harvestbot_ws_ping_ms Latence WebSocket Discord',
    '# TYPE harvestbot_ws_ping_ms gauge',
    `harvestbot_ws_ping_ms ${Math.max(0, Math.round(client.ws.ping))}`,
    '# HELP harvestbot_config_items Objets chargés en configuration',
    '# TYPE harvestbot_config_items gauge',
    `harvestbot_config_items ${config.itemList.length}`,
  ];
  return `${lines.join('\n')}\n`;
}
